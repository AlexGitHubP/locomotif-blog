	...
	<div class='listing-elements-hold'>
		@foreach($items as $key => $item)
			<div class='listing-element {{ $loop->last ? 'lastElement' : '' }}'>
				...
				<div class='listing-box'>
					<p><a href='/admin/items/{{$item->id}}/edit'>{{ $item->category_name }}</a></p>
				</div>
                <div class='listing-box flex06 alignCenter'>
				...
				<div class='listing-box flex02 alignCenter'>
					<div class='more-actions-tab'>
						...
						<ul class='more-list'>
							<li>
								<form action="/admin/items/{{ $item->id }}" method="POST">@method('DELETE') @csrf <input type="submit" value="Șterge"/></form>
							</li>
						</ul>
					</div>
				</div>
